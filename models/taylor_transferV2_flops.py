...
try:
    ...
    print("ptflops is required to calculate FLOPs. Install with `pip install ptflops`.")


def count_flops(model: nn.Module, input_size=(3, 224, 224)):
    """
    Calculate the FLOPs for the given model with the specified input size.

    Args:
        model (nn.Module): The model to profile.
        input_size (tuple): The input size to calculate FLOPs.
    ...
    flops = 2 * macs  # Since each MAC operation is approximately 2 FLOPs
...
if __name__ == "__main__":
    vit_model = vit_small_patch16_224(pretrained=True)
    num_attention_layers = count_attention_layers(vit_model)
    print(f"Number of Attention layers: {num_attention_layers}")

    indices_to_replace = list(range(0, 12))
    order = 1
    vit_model_taylor = replace_attention_with_taylor_by_index(vit_model, indices_to_replace, order)

    # Calculate FLOPs for the original and modified models
    print("Original Model FLOPs:")
    count_flops(vit_model, input_size=(3, 224, 224))
